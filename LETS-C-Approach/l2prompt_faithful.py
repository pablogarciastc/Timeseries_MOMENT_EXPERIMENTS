...
class L2PromptPool(nn.Module):
    ...
    def __init__(self, pool_size=20, prompt_length=5, d_model=512, top_k=5):
        ...
        self.prompts = nn.Parameter(
            torch.randn(pool_size, prompt_length, d_model) / (d_model ** 0.5)
        )

        keys_init = torch.empty(pool_size, d_model)
        nn.init.orthogonal_(keys_init)
        self.keys = nn.Parameter(keys_init * 0.01)
    ...
    def select_prompts(self, x):
        ...
        keys_norm = self.keys / (self.keys.norm(dim=1, keepdim=True) + 1e-8)

        print("query norm: ", query_norm)
        print("keys_norm: ", keys_norm.T)
        ...
        distance = torch.cdist(query, self.keys, p=2)  # shape [batch, pool_size]

        # We want top-K closest (smallest distance), so take smallest values
        _, top_k_indices = distance.topk(self.top_k, dim=1, largest=False)
        ...
        if self.training:
            usage = torch.bincount(top_k_indices.flatten(), minlength=self.pool_size)
            self.prompt_usage += usage.float()
            print(f"Prompt usage this batch: {usage.cpu().numpy()}")
