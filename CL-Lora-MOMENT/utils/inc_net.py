...
def get_backbone(args, pretrained=False):
    ...
    # Coda_Prompt
        ...
        if args["model_name"] == "cllora":
            ...
            raise NotImplementedError("Inconsistent model name and model type")
    print("mdn: ", args["model_name"])
    if args["model_name"] == "cllora_moment":
        ffn_num = args["ffn_num"]
        from backbone import moment_cllora
        from easydict import EasyDict
        tuning_config = EasyDict(
            use_distillation=args["use_distillation"],
            use_block_weight=args["use_block_weight"],
            use_init_ptm=args.get("use_init_ptm", False),  # FIXED: Agregar use_init_ptm
            msa_adapt=args["msa_adapt"],
            msa=args["msa"],
            specfic_pos=args["specfic_pos"],
            general_pos=args["general_pos"],
            ...
            ffn_adapter_scalar="0.1",
            ffn_num=args["ffn_num"],
            d_model=512,  # MOMENT d_model
            vpt_on=False,
            vpt_num=0,
            _device=args["device"][0]
        )

        if "moment-1-small" in name.lower():
            model = moment_cllora.moment_1_small_cllora(tuning_config=tuning_config)
            model.out_dim = 512
        elif "moment-1-large" in name.lower():
            model = moment_cllora.moment_1_large_cllora(tuning_config=tuning_config)
            model.out_dim = 1024
        else:
            raise NotImplementedError("Unknown MOMENT type {}".format(name))
        return model.eval()
...
class PromptVitNet(nn.Module):
    ...
    def extract_vector(self, x):
        return self.backbone(x)['x_embed_norm']
        ...
        if test == False:
            ...
            if self.args["moni_adam"] or (not self.args["use_reweight"]):
                ...
                out = self.fc.forward_diagonal(x_input, cur_task=self._cur_task, alpha=self.alpha,
                                               init_cls=self.init_cls, inc=self.inc, use_init_ptm=self.use_init_ptm,
                                               beta=self.beta)
...
class PabloNet(BaseNet):
    ...
    def feature_dim(self):
        """
        Feature dimension for the full classifier (fc)
        Concatenates features from all tasks + optional init_ptm
        """
        if self.use_init_ptm:
            return self.out_dim * (self._cur_task + 2)
        else:
    ...
    def forward(self, x, test=False):
        if test == False:
            ...
            if self.args.get("moni_adam", False) or (not self.args.get("use_reweight", True)):
                out = self.fc(x_input)
            else:
                out = self.fc.forward_diagonal(
                    x_input, cur_task=self._cur_task, alpha=self.alpha,
