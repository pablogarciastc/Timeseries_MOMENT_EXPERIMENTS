...
class SimpleLinear(nn.Module):
    ...
    '''
    def __init__(self, in_features, out_features, bias=True, to_reduce=False, sigma=True):
...
class CosineLinearFeature(nn.Module):
    ...
    def forward(self, input):
        """
        Standard forward pass for CosineLinearFeature

        Args:
            input: features [batch, in_features]
    ...
    def forward_diagonal(self, input, cur_task, alpha=0., beta=0.0, init_cls=10, inc=10,
                         out_dim=512, use_init_ptm=False):
        """
        Diagonal forward with task-specific reweighting

        Args:
            input: concatenated features [batch, out_dim * num_tasks]
            cur_task: current task index
            alpha: weight for old tasks
            beta: weight for current task
            out_dim: dimension per task (e.g., 512)
            use_init_ptm: whether init pretrained model features are included
        """
        # Handle use_init_ptm offset
        start_offset = 0
        if use_init_ptm:
            start_offset = 1  # Skip first out_dim features

        out_list = []

        # Process each task
        for i in range(cur_task + 1):
            # Determine class range for this task
            if i == 0:
                start_cls = 0
                end_cls = init_cls
            else:
            ...
            # Extract features for task i from input
            feat_start = (start_offset + i) * out_dim
            feat_end = feat_start + out_dim
            input_i = input[:, feat_start:feat_end]
            input_i = F.normalize(input_i, p=2, dim=1)

            # Extract weights for task i classes and task i features
            weight_start = i * out_dim
            weight_end = weight_start + out_dim
            weight_i = self.weight[start_cls:end_cls, weight_start:weight_end]
            weight_i = F.normalize(weight_i, p=2, dim=1)

            # Compute logits for this task's classes
            out_i = F.linear(input_i, weight_i)

            # Apply task-specific weighting
            if i < cur_task:
                task_weight = 1.0 + alpha
            else:
                task_weight = 1.0 + beta

            out_list.append(out_i * task_weight)

        # Concatenate all task outputs
        out_all = torch.cat(out_list, dim=1)

        # Apply reduce_proxies if needed
        if self.to_reduce:
            out_all = reduce_proxies(out_all, self.nb_proxy)
